...
function calculateStandings() {
  ...
  for (
    ...
    games.forEach(([a, b]) => {
      ...
      // If both owners miss the DK contest,
  ...
  const streak = (stat) => {
    ...
    const latest =
      stat.results[stat.results.length - 1];
  ...
  return Object.values(stats)
    .map((stat) => {
      const games =
        stat.w + stat.l + stat.t;

      const winPct = games
        ? (stat.w + 0.5 * stat.t) / games
        : 0;
  ...
  const line = (
    ...
    <div class="team-line ${isWinner ? 'winner' : ''}">
      ...
      <div class="score ${score === null ? 'tbd' : ''}">
        ${score === null ? 'TBD' : score.toFixed(2)}
      </div>
    </div>
  `;
  ...
  $('#seasonPointsLeader').innerHTML =
    ...
    pointsLeader.totalPoints > 0
      ? `
        ...
        <div class="stat-sub">
          <span class="stat-value">
            ${pointsLeader.totalPoints.toFixed(2)}
          </span>
          total DK points
        </div>
      `
  ...
  if (
    ...
    if (liveTeams.length) {
      ...
      if (
        ...
        warnings.push(
          `Teams feed returned ${liveTeams.length} teams instead of ${TEAM_COUNT}.`
        );
      }
    } else {
      warnings.push(
  ...
  if (
    ...
    if (heading) {
      heading.insertAdjacentHTML(
        ...
        `
          <p
            style="color:#d95f13;font-weight:700"
          >
            ${warnings.join(' ')}
          </p>
        `
